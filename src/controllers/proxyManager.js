...
import { saveOrUpdateUser } from './auth.controller.js'
dotenv.config()
...
export async function getAPIKey(req, res) {
  ...
  try {
    ...
    const apiKey = rawData.access_token
    const username = rawData.user?.username || null

    const result = await saveOrUpdateUser(email, apiKey, username)
