...
const elements = {
    dialog : document.getElementById('getAPIKeyDialog'),
    emailInput : document.getElementById('emailInput'),
    passwordInput : document.getElementById('passwordInput'),
    eyeIcon : document.getElementById('eyeIconPassword'),
    cancelChangeIp : document.getElementById('cancelGetAPIKey'),

    title: document.getElementById('getAPIKeyTitle'),
    emailDiv: document.getElementById('emailDiv'),
    register: document.getElementById('register'),
    getKeyBtn: document.getElementById('getKey'),
    viewKeyBtn: document.getElementById('viewKey'),
...
export function showViewKeyDialog(apiKey, eyeIconAPIKey) {
    if (!usingAuth || !apiKey.value || apiKey.type === "text"){
    ...
    elements.title.innerText = 'Re-enter Password to View API Key';
    elements.register.classList.add('invisible');
    elements.viewKeyBtn.classList.remove('hidden');
    elements.getKeyBtn.classList.add('hidden');

    if (authAccount.email) {
        elements.emailInput.value = authAccount.email;
        elements.emailInput.disabled = true;
        elements.emailInput.classList.add('cursor-not-allowed');
        elements.emailDiv.classList.add('opacity-50');
    }
...
export function setUsingAuth(value) {
    usingAuth = value;
}
...
elements.viewKeyBtn.addEventListener("click", async () => {
    ...
    if (!password) {
        ...
        const res = await fetch('/check-password', {
            method: 'POST',
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ password, passwordEn })
        });

        const data = await res.json();

        if (data.valid) {
            showToast("Password correct! Showing API key...", "success");
            closeAPIKeyDialog();
            elements.apiKeyInput.type = "text";
            elements.eyeIconAPIKey.innerHTML = openEyePath;
        } else {
            showToast("Incorrect password <br> Please try again or get other API KEY", "error");
            closeAPIKeyDialog();
        }
    }
});