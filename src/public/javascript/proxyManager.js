...
function bindEvents() {
  ...
  elements.loadBtn.addEventListener('click', getFromDb)
  elements.saveBtn.addEventListener('click', saveToDb)
}
...
async function getFromDb() {
  showToast('Getting from database...', 'loading')

  const apiKeyString = elements.apiKey.value.trim()

  try {
    const response = await fetch('/proxy/get-from-db', {
      method: 'POST', // backend expects apiKey from body
      headers: { 'Content-Type': 'application/json' },
    ...
    if (response.ok && response.status === 200) {
      ...
      changeToToast('Loaded from DB!', 'Getting from database...', 'success')
    } else {
      switch (response.status) {
        case 401:
          changeToToast('Wrong API KEY!', 'Getting from database...', 'error')
          break
        default:
          changeToToast(
            `Error ${response.status}`,
    ...
    console.error(err)
    changeToToast('Fail to get from DB!', 'Getting from database...', 'error')
  }
}
...
async function saveToDb() {
  ...
  try {
    const response = await fetch('/proxy/sync-to-db', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        apiKey: apiKeyString,
    ...
    if (response.ok && response.status === 200) {
      ...
      changeToToast(
        `DB Synced<br>
        <span class="text-text-toast-success">Insert: ${result.inserted}</span><br>
        <span class="text-text-warning">Update: ${result.updated}</span><br>
        <span class="text-text-toast-error">Delete: ${result.deleted}</span>`,
        'Saving to database...',
        'success'
      )
    } else {
      switch (response.status) {
        case 401:
          changeToToast('Wrong API KEY!', 'Saving to database...', 'error')
          break
        default:
          changeToToast(
            `Error ${response.status}`,
    ...
    console.error(err)
    changeToToast('Fail to save to DB!', 'Saving to database...', 'error')
  }
}
